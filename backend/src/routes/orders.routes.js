...
router.put("/:id", verifyToken, async (req, res, next) => {
  try {
    ...
    // User chỉ có thể hủy đơn (status = cancelled) hoặc xác nhận thanh toán (status = paid/banked), admin có thể update bất kỳ status nào
    if (req.role !== "admin" && req.body.status && req.body.status !== "cancelled" && req.body.status !== "banked" && req.body.status !== "paid") {
      return res.status(403).json({ message: "You can only cancel your order or confirm payment" });
    }
