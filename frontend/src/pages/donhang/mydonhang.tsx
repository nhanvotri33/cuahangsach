...
const MyDonHang: React.FC = () => {
  ...
  const mutationCancelOrder = useMutation(
    async (orderId: string) => {
      ...
      // Gửi đầy đủ thông tin khi cập nhật
    ...
    },
    {
      ...
      onError: (error: any) => {
        ...
        alert("Không thể hủy đơn hàng: " + (error?.message || "Vui lòng thử lại"));
      },
    }
  );
                  ...
                  {enrichOrderItems(selectedOrder.items).map((item) => (
                    ...
                    >
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-black/60">Đơn giá: {Number(item.price).toLocaleString("vi-VN", { maximumFractionDigits: 0 })} VND</p>
                      </div>
                      <div className="text-right">
                        <p className="text-lg font-bold text-orange-600">
                          {Number(item.quantity * item.price).toLocaleString("vi-VN", { maximumFractionDigits: 0 })} VND
                        </p>
                      </div>
                    </div>
                  ))}
              ...
              <div className="mt-6 pt-4 border-t border-orange-200">
                <div className="flex justify-between items-center mb-4">
                  ...
                  <span className="text-2xl font-bold text-orange-600">
                    {Number(selectedOrder.total_amount).toLocaleString("vi-VN", { maximumFractionDigits: 0 })} VND
                  </span>
                </div>
                
                {/* Nút thanh toán nếu status pending và payment_method là bank_transfer */}
                {selectedOrder.status === "pending" && selectedOrder.payment_method === "bank_transfer" && (
                  <div className="mt-4">
                    <button
                      onClick={() => {
                        setSelectedOrder(null);
                        navigate(`/payment?orderId=${selectedOrder.order_id}`);
                      }}
              ...
              <div className="mb-4 space-y-2">
                <p className="text-black">
                  ...
                  <span className="text-orange-600 font-bold">
                    {Number(order.total_amount).toLocaleString("vi-VN", { maximumFractionDigits: 0 })} VND
                  </span>
                </p>
                <p className="text-black">
                  <span className="font-semibold">Phương thức thanh toán:</span>{" "}
